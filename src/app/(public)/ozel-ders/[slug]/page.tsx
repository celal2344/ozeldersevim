...
export default async function LessonLandingPage({ params }: PageProps) {
  ...
  const { data: teachers, fallback } = await searchTeachers({ lesson: slug, pageSize: 4 });
  const showsFallbackTeachers = fallback?.reason === "filters_relaxed";
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-8">
            ...
            <h2 className="mt-1 text-2xl font-extrabold text-brand-navy">
              {showsFallbackTeachers ? "Önerilen Öğretmenler" : `${lesson.name} Öğretmenleri`}
            </h2>
            {showsFallbackTeachers ? (
              <p className="mt-2 text-sm text-muted-foreground">
                Bu ders için tam eşleşme bulunamadı. Yayındaki diğer öğretmenleri öneriyoruz.
              </p>
            ) : null}
          </div>
          ...
          {teachers.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center gap-4 p-5">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-bold text-brand-navy transition-colors group-hover:text-brand-orange">
                          {teacher.fullName}
                        </p>
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center shadow-sm">
              <p className="text-lg font-bold text-brand-navy">Bu ders için yayınlanmış öğretmen hazırlanıyor.</p>
              <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                İlanlar yayına alındığında burada listelenecek. Şimdilik tüm öğretmenleri inceleyebilir veya daha sonra tekrar kontrol edebilirsin.
              </p>
              <Link
                href="/ogretmen-bul"
                className="mt-5 inline-flex items-center gap-2 rounded-xl bg-brand-orange px-5 py-3 text-sm font-bold text-white shadow-lg shadow-orange-200 transition-all hover:scale-[1.02] hover:bg-orange-400"
              >
                Tüm Öğretmenleri Gör <ArrowRightIcon className="size-4" aria-hidden="true" />
              </Link>
            </div>
          )}
        </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl bg-slate-50 p-4">
                    ...
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">
                      Türkiye&apos;nin her yerinden öğretmenlerle bağlantı kur. Kendi ortamında ders al.
                    </p>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-4">
                    <p className="font-semibold text-brand-navy">Yüz Yüze Ders</p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">
                      Şehrindeki öğretmenlerle daha yüksek etkileşimli dersler planla.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="px-6 py-5">
                  <div className="flex flex-col gap-4">
                    {[
                      ...
                      "Ders talebi gönder.",
                      "Öğretmen kabul edince iletişim bilgilerin paylaşılır.",
                      "Ders programınızı birlikte planlayın.",
                    ].map((step, index) => (
